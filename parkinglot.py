...
class ParkingLotTopo(Topo):
	...
	s = {}
	h = {} 
        s[1] = self.addSwitch('s1')
        h[1] = self.addHost('h1', **hconfig)
        self.addLink(receiver, s[1],
                      port1=0, port2=uplink, **lconfig)
        self.addLink(h[1], s[1],
                      port1=0, port2=hostlink, **lconfig)

	for x in range(2, n+1):
	    s[x] = self.addSwitch('s'+str(x))
	    h[x] = self.addHost('h'+str(x), **hconfig)
	    self.addLink(s[x-1], s[x], port1=downlink, port2=uplink, **lconfig)
	    self.addLink(h[x], s[x], port1=0, port2=hostlink, **lconfig)
...
def run_parkinglot_expt(net, n):
    ...
    for x in range(2, n+1):
        host = net.getNodeByName('h'+str(x))
        host.sendCmd('iperf -c %s -p %s -t %d -i 1 -yc > %s/iperf_%s.txt' % (recvr.IP(), 5001, seconds, args.dir, 'h'+str(x)))
